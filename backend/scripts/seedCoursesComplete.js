/**
 * Create a demo instructor user and seed courses
 */
...
const demoInstructor = {
  ...
  profile: {
    ...
    avatar: 'https://ui-avatars.com/api/?name=Demo+Instructor&size=200'
  },
  instructorProfile: {
    bio: 'Passionate about education and helping students succeed. Experienced in multiple domains including technology, business, and personal development.',
    expertise: ['Web Development', 'Data Science', 'Digital Marketing', 'Design', 'Cloud Computing'],
    experience: 'Over 10 years of professional teaching experience',
    qualifications: ['Master\'s in Computer Science', 'Bachelor\'s in Education'],
    socialLinks: {
      website: 'https://nexed.com',
      linkedin: 'https://linkedin.com/in/demoinstructor',
      twitter: 'https://twitter.com/demoinstructor'
    },
    isApproved: true,
    earnings: {
      total: 0,
...
const mockCourses = [
  ...
  },
  {
    ...
    originalPrice: 3499,
    discount: 48,
    thumbnail: "https://images.unsplash.com/photo-1452780212940-6f5c0d14d848?w=800&q=80",
    learningOutcomes: [
...
async function seedAll() {
  try {
    ...
    let instructor = await User.findOne({ email: demoInstructor.email });
    ...
    if (instructor) {
      ...
      // Create instructor
      instructor = await User.create(demoInstructor);
    ...
    const coursesWithInstructor = mockCourses.map(course => {
      ...
      courseData.totalDuration = totalDuration * 60;
    ...
    insertedCourses.forEach((course, index) => {
      ...
      if (course.price === 0) freeCount++;
      else paidCount++;
      
      console.log(`${index + 1}. ${course.title}`);
      console.log(`   📁 Category: ${course.category} | 📊 Level: ${course.level}`);
      console.log(`   💰 Price: ${priceDisplay} | 📚 Lectures: ${course.totalLectures} | ⏱️  Duration: ${Math.floor(course.totalDuration / 60)} min`);
